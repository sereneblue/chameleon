...
devices.mobile.ios2 = (devices.mobile.ios1.map((d: Device) => {
  return { build :  '16G130', name :  d.name, viewport :  d.viewport };
}) as any).concat([
  {
    name :  'iPhone XS',
    build :  '16G130',
...
devices.mobile.ios3 = (devices.mobile.ios2.map((d: Device) => {
  return { build :  '17B102', name :  d.name, viewport :  d.viewport };
}) as any).concat([
  {
    name :  'iPhone 11',
    build :  '17B102',
...
devices.tablet.ios2 = (devices.tablet.ios1.map((d: Device) => {
  return { build :  '16G130', name :  d.name, viewport :  d.viewport, deviceScaleFactor :  d.deviceScaleFactor };
}) as any).concat([
  {
    name :  'iPad Air (2019)',
    build :  '16G102',
...
devices.tablet.ios3 = devices.tablet.ios2.map((d: Device) => {
  return { build :  '17B102', name :  d.name, viewport :  d.viewport, deviceScaleFactor :  d.deviceScaleFactor };
}) as any;
