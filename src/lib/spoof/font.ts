...
const FONTS = {
  ...
  mac1: COMMON_FONTS_MAC.concat(['Hiragino Kaku Gothic ProN']),
  mac2: COMMON_FONTS_MAC.concat(['Farisi']),
  mac3: COMMON_FONTS_MAC.concat(['Farisi', 'Galvji', 'Kohinoor Gujarati', 'MuktaMahee', 'Noto Sans Kannada', 'Noto Sans Myanmar', 'Noto Sans Oriya']),
  ...
  ios1: COMMON_FONTS_IOS.concat(['Bangla Sangam MN', 'Gujarati Sangam MN', 'Gurmuhki MN', 'Heiti SC', 'Heiti TC', 'Kannada Sangam NM', 'Oriya Sangam MN', 'Telugu Sangam MN']),
  ...
  ios3: COMMON_FONTS_IOS.concat(['Apple Symbols', 'Galvji', 'Kohinoor Gujarati', 'Mukta Mahee', 'Noto Sans Myanmar', 'Noto Sans Oriya']),
  and1: COMMON_FONTS_ANDROID,
  and2: COMMON_FONTS_ANDROID.concat(['Coming Soon']),
  and3: COMMON_FONTS_ANDROID.concat(['Coming Soon']),
  and4: COMMON_FONTS_ANDROID.concat(['Coming Soon']),
};
export const WHITELIST_FONTS = Object.keys(FONTS)
  .reduce((fonts, key) => fonts.concat(FONTS[key]), [] as string[])
  .filter((font, i, fonts) => fonts.indexOf(font) === i)
  .sort();
...
export default {
  ...
  data: `
    ...
    const DEFAULT_FONT = ${JSON.stringify(DEFAULT_FONT)};
    ...
    let excludedFonts = (CHAMELEON_SPOOF.get(spoofContext).whitelistFontExclude || []).map(f => f.toLowerCase());
    let selectedFonts = FONTS[CHAMELEON_SPOOF.get(spoofContext).profileOS].map(f => f.toLowerCase()).filter(f => !excludedFonts.includes(f)).concat(DEFAULT_FONT);
        ...
        for (let i = 0; i < spoofContext.document.styleSheets.length; i++) {
          for (let j = 0; j < spoofContext.document.styleSheets[i].cssRules.length; j++) {
            ...
            if (spoofContext.document.styleSheets[i].cssRules[j].type === CSSRule.FONT_FACE_RULE) {
              ...
              if (!excludedFonts.includes(loaded) && !selectedFonts.includes(loaded)) {
    ...
    spoofContext.document.fonts.onloadingdone = function(evt) {
      for (let f of evt.fontfaces) {
        ...
        if (!excludedFonts.includes(fontFamily) && !selectedFonts.includes(fontFamily)) {
