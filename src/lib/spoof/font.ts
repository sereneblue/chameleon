			...
			try {
				for (let i = 0; i < document.styleSheets.length; i++) {
					for (let j = 0; j < document.styleSheets[i].cssRules.length; j++) {
						...
						if (document.styleSheets[i].cssRules[j].type === CSSRule.FONT_FACE_RULE) {
							let loaded = document.styleSheets[i].cssRules[j].style.getPropertyValue("font-family").toLowerCase();
